...
contract TheWeekendTest is Test, IERC1155Receiver {
    ...
    function test_IsWeekend() public view {
        ...
        // Saturday timestamp
        uint256 saturdayMorning = 1688803200; // Saturday 8am EST
        // Sunday timestamp
        uint256 sundayEvening = 1688938800; // Sunday 6pm EST

        // Test weekend days
        assertTrue(theWeekend.isWeekEnd(saturdayMorning), "Saturday should be weekend");
        assertTrue(theWeekend.isWeekEnd(sundayEvening), "Sunday should be weekend");

        // Test non-weekend days
        uint256 mondayMorning = 1689001200; // Monday 8am EST
        uint256 thursdayNoon = 1688652000; // Thursday 12pm EST
        uint256 fridayNoon = 1688738400; // Friday 12pm EST

        assertFalse(theWeekend.isWeekEnd(mondayMorning), "Monday should not be weekend");
        assertFalse(theWeekend.isWeekEnd(thursdayNoon), "Thursday should not be weekend");
        assertFalse(theWeekend.isWeekEnd(fridayNoon), "Friday should not be weekend");
    }
